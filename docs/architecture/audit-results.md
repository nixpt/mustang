...
| **2026-06-22T03:39:30Z** (post-trim cadence re-run) | `be399d9` | **PASS** (1.0s) | **PASS** (0s) | **PASS** (0s) | All 4 boundaries HOLDS; CSS-aux 0 matches; **0 broad-umbrella matches** (`^dioxus`, `^dioxus-`, `^dioxus_`, `^taffy`, `^parley` etc. all yield 0) | **NONE detected** vs `2026-06-22T01:33:52Z` baseline AND unchanged vs the `2026-06-22T02:59:20Z` row. Dep-graph fingerprint stable at **4217** rows; no new `[features]` or `[dependencies]` entry landed between this trim run and the prior cadence row (the `EffectType::DropShadow` / `EffectType::CanonicalFilter` / `DropShadowParams` / `CanonicalFilter` additions at `c53fe31f` are pure-Rust types + their tests + parser — no row reaches the dep tree). **No compile-side gate flips**: this trim is comment-only (a single 3-line doc-comment reword in `parse_canonical_filter`'s helper about CSS Filter Effects spec column ordering — no code path change), so the 3 cargo-check gate timings remain at parity with the `2026-06-22T02:59:20Z` row. **Test count: 52** at this commit (was 38 before `c53fe31f`'s `DropShadow` + `CanonicalFilter` introduction; the +14 net tests come from that prior commit, not from this trim). The test count is surfaced in this row as a non-doctrinal-but-artifact-relevant dimension so future audits can detect test-count regressions even when the dep-graph fingerprint is stable. | **4217** (matches the `2026-06-22T02:59:20Z` row verbatim; this trim touches `src/compositor/mod.rs`'s comment only, not the `[features]` block or `[dependencies]` block, so the tree-size fingerprint is preserved). |
